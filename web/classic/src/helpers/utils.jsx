...
export function normalizeSystemName(systemName) {
  const trimmed = typeof systemName === 'string' ? systemName.trim() : '';
  if (!trimmed) return 'Tokenflow';
  return trimmed.replace(/\s+/g, '').toLowerCase() === 'vibeapi'
    ? 'Tokenflow'
    : trimmed;
}
...
export function getSystemName() {
  ...
  return normalizeSystemName(system_name);
}

export function getLogo() {
  let logo = localStorage.getItem('logo');
  if (!logo) return '/logo.png?v=3';
  return logo;
}
